...
int WINAPI JACIC_CheckHashValue(const char *checkFile)
{
    ...
    HashBuffer *dateBuff = NULL;
    APP5Item app5ImageHash = {JACIC_FLAG_OFF, NULL};
    APP5Item app5DateHash = {JACIC_FLAG_OFF, NULL};
    size_t readJpegBufferMemorySize;
    ...
    /*
     * APP5 セグメント (DCPMI) 内のハッシュ値を確認、取得
     */
    dcpmiRet = getDCPMIHashValue(readJpegBuffer, &app5ImageHash, &app5DateHash, seek);

    if(dcpmiRet != FUNCTION_SUCCESS)
    {
        if(dcpmiRet != HASH_NOT_EXISTS &&           /* DCPMI は見つかったがハッシュ値は格納されていなかった場合 */
                dcpmiRet != INCORRECT_APP5_FORMAT)  /* APP5 領域は存在したが DCPMI ではなかった */
        {
            ret = dcpmiRet;
            goto FINALIZE;
        }
        ...
        /*
         * APP5 セグメント (RMETA) 内のハッシュ値を確認、取得
         */
        rmetaRet = getRMETAHashValue(readJpegBuffer, &app5ImageHash, &app5DateHash, seek);
        
        if(rmetaRet != FUNCTION_SUCCESS &&
                rmetaRet != HASH_NOT_EXISTS)
        {
            if(rmetaRet == APP5_NOT_EXISTS ||           /* RMETA が見つからなかった */
                    rmetaRet == INCORRECT_APP5_FORMAT)  /* APP5 領域は存在したが RMETA ではなかった */
            {
                /* RMETA のエラーで上書きしてしまうため DCPMI のエラーを利用する */
                ret = dcpmiRet;
            }
            else
            {
