...
/*!
@name APP5 セグメント RMETA/DCPMI 定義サイズに関する定義
@{
*/
...
#define BYTE_SIZE_DCPMI_CHARACTER_CODE         ((size_t)8)     /*!< @brief メタデータ内文字コード定義のバイト数 */
...
#define BYTE_SIZE_DCPMI_IDENTIFIER_SIZE        ((size_t)6)     /*!< @brief APP5 DCPMI 識別子のバイト数 */
/*!
@}
...
/*! DCPMI メタバージョンの値 */
static const unsigned short APP5_DCPMI_VERSION = 0x0110;
...
{
    0xFF, 0xE5, 0x0D, 0x78, 0x44, 0x43, 0x50, 0x4D, 0x49, 0x00,
    0x4D, 0x4D, 0x01, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A,
...
/*! メタデータ識別名「DCPMI」（終端文字含む） */
static const char IDENTIFIER_DCPMI[] =
{
    0x44, 0x43, 0x50, 0x4D, 0x49, 0x00
};
...
*/
static int getRMetaItemValue(const char *searchTitle, APP5Item  *retItemValue, HashBuffer *rMetaItemText, HashBuffer *rMetaItemText128, unsigned short entryCount)
{
    int ret;
...
@retval OTHER_ERROR メモリ確保に失敗した場合
*/
static int getRMetaHashValue(JpegBuffer *src, APP5Item *retImageHash, APP5Item *retDateHash, unsigned long *seek, unsigned short entryCount)
{
    int ret = FUNCTION_SUCCESS;
    int funcRet;
...
@param [out] retDateHash 取得したハッシュ値（撮影日時）を格納する変数
@param seek 走査開始位置
@param targetIdentifier 探査対象となる APP5 識別子
@param targetMetaVersion 探査対象となる APP5 メタバージョン
@retval FUNCTION_SUCCESS 正常終了
@retval INCORRECT_PARAMETER 不正な引数が指定された場合
@retval INCORRECT_APP5_FORMAT APP5 領域が不正な形式の場合
...
@retval OTHER_ERROR メモリ確保に失敗した場合
*/
static int getAPP5HashValue(JpegBuffer *src, APP5Item *retImageHash, APP5Item *retDateHash, unsigned long seek, const char *targetIdentifier, size_t identifierSize, unsigned short targetMetaVersion)
{
    int ret = FUNCTION_SUCCESS;
    int app5ExistsResult;
    int nextSegmentNumber = -1;                    /* 次のセグメントの番号：初回は -1 */
    char charCode[BYTE_SIZE_DCPMI_CHARACTER_CODE]; /* メタデータ内の文字コード定義 */
    ...
    unsigned short dcpmiDataOffset;                /* Byte Order から数えた DCPMI Data へのオフセット */
    ...
    while(1)
    {
        ...
        /* 画像からAPP5セグメントを検出する */
        app5ExistsResult = checkApp5Exists(src, &seek);
        
        if(app5ExistsResult != FUNCTION_SUCCESS)
        {
            /*
             * 『 APP5 が存在し、かつ targetIdentifier とは異なる識別子を持つ』場合に結果を上書きしてしまうため、
             * 前回検索時に何らかの失敗があった場合はそれを採用する
             */
            if(ret == FUNCTION_SUCCESS)
            {
                ret = app5ExistsResult;
            }
            break;
        }
        ...
        /* セグメント識別名の取得（targetIdentifier ではない場合はこのセグメントを無視して次の APP5 セグメントへ） */
        ret = checkIdentifier(targetIdentifier, src, &seek, identifierSize);
        
        if(ret != FUNCTION_SUCCESS)
        {
            if(ret == INCORRECT_IDENTIFIER)
            {
                /* 識別子が異なる場合はフォーマットが異なると同義とする */
                ret = INCORRECT_APP5_FORMAT;
            }
        ...
        /* seek の記憶（Byte Order に Data Offset を足した値が Data の開始位置となるため） */
        byteOrderStartIndex = seek;
        
        /* Byte Order の取得（MM（ビッグエンディアン）ではない場合はこのセグメントを無視して次の APP5 セグメントへ） */
        ...
        /* メタバージョンの取得（0x0110 でない場合はこのセグメントを無視して次の APP5 セグメントへ） */
        metaVersion = getMetaVersion(src, seek);
        
        if(metaVersion != targetMetaVersion)
        {
            /* メタバージョンが異なる場合はフォーマットが異なるものとして扱う */
            ret = INCORRECT_APP5_FORMAT;
            
            /* このセグメントを無視して次の APP5 セグメントを探す */
            seek = app5EndNextIndex;
            continue;
        }
        ...
        dcpmiDataOffset = getRMetaDataOffset(src, seek);
        
        /* オフセット定義のバイトサイズ分走査位置を進める */
        seek += BYTE_SIZE_RMETA_DATA_OFFSET;
        
        if(dcpmiDataOffset < seek - byteOrderStartIndex)
        {
        ...
        seek = byteOrderStartIndex + dcpmiDataOffset;
...
{
    return getAPP5HashValue(src, retImageHash, retDateHash, seek, IDENTIFIER_RMETA, BYTE_SIZE_RMETA_IDENTIFIER_SIZE, APP5_RMETA_VERSION);
}
...
{
    return getAPP5HashValue(src, retImageHash, retDateHash, seek, IDENTIFIER_DCPMI, BYTE_SIZE_DCPMI_IDENTIFIER_SIZE, APP5_DCPMI_VERSION);
}
